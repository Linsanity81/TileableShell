mlStruct = parseXML('3^4-6_11844.xml');
...
for i = 1 : length(tileInfo)
    ...
    currData = sscanf(currStr, "%s %f %f %s %f %f %s %f %f %s ");

    if (currData(10) ~= 'Z')
        currData = sscanf(currStr, "%s %f %f %s %f %f %s %f %f %s %f %f %s %f %f %s %f %f %s");
        ...
        if (sum(n) > 0)
            hexa(end + 1, :) = [v1_index, v2_index, v3_index, v4_index, v5_index, v6_index];
        else
            hexa(end + 1, :) = [v6_index, v5_index, v4_index, v3_index, v2_index, v1_index];
        end
        ...
        if (sum(n1) > 0)
            triangle(end + 1, :) = [v1_index, v2_index, v3_index];
        else
            triangle(end + 1, :) = [v3_index, v2_index, v1_index];
        end
